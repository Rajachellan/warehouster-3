...
const P2 = "#0f2040";
const G = "#D4AF37";
const P12 = "rgba(10,20,40,0.12)";
const P08 = "rgba(10,20,40,0.08)";
const P05 = "rgba(10,20,40,0.05)";
...
const REASONS = [
  {
    ...
    BannerIcon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="#ffffff" strokeWidth="1.6" className="w-[18px] h-[18px]">
...
function Pill({ label }: { label: string }) {
  return (
    <div
      ...
      style={{ borderColor: P12, background: P05, color: G }}
    >
...
function ReasonCard({ r, index }: { r: typeof REASONS[0]; index: number }) {
  ...
  return (
    <motion.div
      ...
      style={{
        border: `1.5px solid ${hov ? P12 : P08}`,
        boxShadow: hov
          ? `0 12px 48px rgba(10,20,40,0.08), 0 2px 14px rgba(10,20,40,0.05)`
    ...
    >
      ...
      />
      {/* Bottom primary sweep */}
      <div
        className="absolute bottom-0 left-0 h-[2.5px] rounded-b-[24px]"
        style={{
          width: hov ? "100%" : "0%",
          background: `linear-gradient(to right, ${P2}, rgba(15,32,64,0.2))`,
          transition: "width .5s cubic-bezier(.22,1,.36,1)",
        }}
      />
      {/* Corner bracket TL */}
      <div className="absolute top-[16px] left-[16px]" style={{ width: hov ? 26 : 18, height: hov ? 26 : 18, borderTop: `1.5px solid ${hov ? P12 : P08}`, borderLeft: `1.5px solid ${hov ? P12 : P08}`, transition: "all .3s" }} />
      {/* Corner bracket BR */}
      <div className="absolute bottom-[16px] right-[16px]" style={{ width: hov ? 26 : 18, height: hov ? 26 : 18, borderBottom: `1.5px solid ${hov ? P12 : P08}`, borderRight: `1.5px solid ${hov ? P12 : P08}`, transition: "all .3s" }} />
      ...
      <div className="flex flex-col flex-1 p-7">
        ...
        >
          ...
          <span className="font-serif font-black leading-none select-none flex-shrink-0 relative z-10" style={{ fontSize: 32, color: "rgba(255,255,255,0.15)", letterSpacing: "-0.03em" }}>{r.num}</span>
          <div className="w-px h-9 flex-shrink-0 relative z-10" style={{ background: "rgba(255,255,255,0.15)" }} />
          <div className="w-[38px] h-[38px] flex items-center justify-center rounded-[10px] flex-shrink-0 relative z-10" style={{ background: "rgba(255,255,255,0.08)", border: `1px solid rgba(255,255,255,0.15)`, transition: "all .3s" }}>
            {r.BannerIcon}
          </div>
          <span className="text-[11px] font-black uppercase tracking-[0.28em] leading-[1.5] relative z-10" style={{ color: "rgba(255,255,255,0.55)" }}>{r.phase}</span>
        </div>

        {/* Category pill */}
        <div
          className="inline-flex items-center self-start px-[10px] py-[4px] rounded-full border text-[11px] font-black uppercase tracking-[0.28em] font-sans mb-[14px]"
          style={{ borderColor: hov ? P12 : P08, background: hov ? P05 : P05, color: hov ? P : N40, transition: "all .3s" }}
        >
          {r.label}
        </div>
        ...
        {/* Gold rule */}
        <div className="h-px mb-3" style={{ width: hov ? 40 : 22, background: P, opacity: hov ? 0.2 : 0.1, transition: "width .35s, opacity .35s" }} />

        {/* Description */}
        <p className="text-[14px] leading-[1.85] font-sans flex-1" style={{ color: N55 }}>
          {r.desc}
        </p>
      </div>
    </motion.div>
...
function MiniStat({ val, label, index }: { val: string; label: string; index: number }) {
  ...
  return (
    <motion.div
      ...
      style={{
        border: `1.5px solid ${hov ? P12 : P08}`,
        boxShadow: hov ? `0 4px 20px rgba(10,20,40,0.06)` : "none",
        transition: "border-color .3s, box-shadow .3s",
      }}
    >
      <span className="font-serif font-black leading-none flex-shrink-0" style={{ fontSize: 26, color: P, minWidth: 56 }}>
        {val}
      </span>
      <div className="w-px h-8 flex-shrink-0" style={{ background: P08 }} />
      <span className="text-[13px] font-black uppercase tracking-[0.2em] leading-[1.55] whitespace-pre-line" style={{ color: N55 }}>
        {label}
      </span>
    </motion.div>
  );
...
function SuccessCard() {
  return (
    <motion.div
      ...
      style={{ border: `1px solid ${P08}`, background: "#f8f9fb" }}
    >
      {/* Header */}
      <div className="flex items-center gap-3 mb-5">
        <div className="w-[38px] h-[38px] flex items-center justify-center rounded-[10px] flex-shrink-0" style={{ background: P05, border: `1px solid ${P08}` }}>
          <Monitor size={16} style={{ color: P }} strokeWidth={1.6} />
        </div>
        <div>
          <p className="text-[12px] font-black uppercase tracking-[0.32em] font-sans" style={{ color: N40 }}>Proof of Execution</p>
          <p className="font-serif font-black text-[16px] uppercase tracking-tight" style={{ color: P }}>Redhills, Chennai</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-5">
        {[{ end: 25, suffix: " acres", label: "Greenfield Park" }, { end: 14, suffix: " months", label: "Fully Leased" }].map((s, i) => (
          <div key={i} className="px-4 py-4 rounded-xl" style={{ background: P05, border: `1px solid ${P08}` }}>
            <p className="font-serif font-black leading-none mb-1" style={{ fontSize: 28, color: P }}>
              <CountUp end={s.end} suffix={s.suffix} />
            </p>
            <p className="text-[11px] font-black uppercase tracking-[0.22em] font-sans" style={{ color: N40 }}>{s.label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex items-start gap-3">
        <TrendingUp size={13} strokeWidth={1.6} style={{ color: N40, flexShrink: 0, marginTop: 2 }} />
        <p className="text-[14px] font-sans leading-[1.75]" style={{ color: N55 }}>
          Fully leased to institutional-grade occupiers — a validated proof-point of our integrated development approach.
        </p>
      </div>

      {/* Bottom rule */}
      <div className="mt-5 h-px" style={{ background: `linear-gradient(to right, ${P08}, transparent)` }} />

      {/* Trusted by */}
      <div className="flex items-center gap-3 flex-wrap mt-4">
        <span className="text-[12px] font-black uppercase tracking-[0.35em] font-sans" style={{ color: N40 }}>Trusted by</span>
        {TRUSTED.map((name) => (
          <div
            key={name}
            className="inline-flex items-center gap-2 px-[14px] py-[5px] rounded-full border text-[12px] font-black uppercase tracking-[0.18em] font-sans cursor-default transition-all duration-250 hover:border-primary/20 hover:bg-primary/[0.03]"
            style={{ border: `1px solid ${P08}`, background: P05, color: N40 }}
          >
            <span className="w-[4px] h-[4px] rounded-full flex-shrink-0" style={{ background: P, opacity: 0.4 }} />
            {name}
          </div>
        ))}
      </div>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
            ...
            <p
              className="text-base leading-relaxed max-w-[260px] md:text-right font-sans"
              style={{ color: N55, borderRight: `2px solid ${P08}`, paddingRight: 16 }}
            >
              Unmatched combination of institutional expertise, occupier relationships, and execution discipline.
            </p>
          </div>
          {/* Gold gradient rule */}
          <div className="mt-10 h-px" style={{ background: `linear-gradient(to right, ${P08}, transparent)` }} />
        </motion.div>
